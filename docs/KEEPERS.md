...
3. **Where a hand goes** (K8): grown dragons are stroked on the back of the head, the neck or the back; babies are
   patted on the rump and sleepy dragons stroked on the back, never the head. At this size a dragon's eye is most of
   the side of its head and a keeper's fist is 9 or 10 px: there is no room for a head pat that keeps off the eye.
4. **The pace** (7): needs fill in 36 to 88 s, a feed takes about 20 s from the kitchen and back, and the yard is
...
(`src/care/acts.ts`, `keeper.ts`, `dragon.ts`), the yard (`src/care/yard.ts`), and the gallery views `keepers`,
`care`, `careaudit`, `yard` and `yardaudit`. `npm run check` runs the keeper palette gates (144 of 144 pass, counted
apart from the dragons' 2255) and seven new smoke cases, the two care audits among them (every act on all 28 looks:
112 runs; and every act the yard plays in two and a half minutes). `npm run shots` renders the keeper sheets. The
...
| K3 | **People in the dragons' house style**: the 1 px `#1a1018` ink, cel bands lit from the top-left, the mark floor (bible 5.2). Faces are never angry (the dragons' D18): the keepers' face set is neutral, smile, happy, closed, oh, aww and shh. The elder keeper follows the elder dragons' rule (D21): unhurried, never frail; no stoop, no cane. | A cozy game; the elder is drawn as a reward, not decline, for people too. |
...
| K11 | **The yard**: needs rise and show (7.1); a director sends the keeper whose job it is to the neediest dragon (7.2); keepers wait at stations at the sides, walk round behind a dragon rather than through it, and walk into and out of the scene on a diagonal. | A living scene, with nothing in it the player has to run. |
...
Two colours moved to pass: **Bea's blush** (a coral one sat 8 % and 25 deg from her skin and vanished; now a
raspberry rose, `#e8508a`, 48 deg away) and **Tomas's denim** (at the value of Iris's mauve cardigan the two tops
merged under deuteranopia, then protanopia; now `#3d5f94`). In short (`npm run palette` prints every pair with its
numbers, after the dragons' report):
...
KEEPERS: PASS  144 of 144 gates passed
```
...
for buckled boots), and the **tools**: the clay bowl (the one bowl the dragons eat from, `src/art/props.ts`, carried
as a front accessory so the hands hold it rather than cover it) and Tomas's brush, held by its back with the
bristles down whatever the arm does (out of the fist along the forearm, a handle and a block read as a knife, and a
T-head as a toilet brush poking the dragon).
...
| Anim | What it is |
|---|---|
| idle (150 f) | a breath, the weight shifting foot to foot, each keeper's own hands: Bea's clasped on her apron |
| walk, carry, tiptoe | a heel-toe walk, stance 60 % / swing 40 %, the stance foot planted; carry holds the bowl at the waist; tiptoe (away from a sleeping dragon) keeps a finger at the lips |
...
| shh, cheer, wave | a finger to the lips; a fist pumped up in front of the face and a hop (the elder claps three times instead: D21); a wave |
...
- **Feed** (Bea). She carries a bowl the size of the dragon's (its eat spot, `bowlFor`) to a begging dragon, kneels
  and sets it down a little way out in front of it (the gap is solved so nothing of her covers the begging dragon's
  eye), steps back, and the dragon walks up to it, eats two bites and is happy; she cheers, the dragon turns and trots
  off, and she comes back for the empty bowl.
- **Pet** (Pip with a baby, Bea or anyone in the gallery) and **groom** (Tomas, his brush bristles-down on the mark).
  The keeper settles into its pose, eases its hand on, strokes four times while the dragon plays its pet loop, lets go,
  and watches (kneeling or standing) the happy that thanks it; then rises and goes.
...
K9. A keeper walks along the floor on its anim's own root motion (its planted feet hold still) and into or out of the
scene on a diagonal that arrives with it; a walk that would cross a dragon's body goes round **behind** it (the dragon
hides the keeper's feet as it passes). The act gives the dragon back as the keeper walks off, so the yard can send it
home while the keeper leaves.
...
1. **Marks** in order (K8). A mark is an arc of the cranium circle, a stretch of the neck's top contour or of the back
   (`craniumPoint`, `neckTop`, `backTop` in `src/care/dragon.ts`); the hand's centre rests a hand's radius out, a
   brush's fist 8 px over it. A quick test first drops a mark whose fist could never keep off the eye.
...
3. **Spots** 1 px apart outward from where the mark sits at 0.7 of the reach. A spot must **reach**: every end of the
   ...
   by frame, asleep). The engine's fist is modelled exactly (a rounded block -0.6 r to +1.6 r along the forearm, the
   thumb's ball) as a cheap first test before any drawing.
4. The first spot that passes wins; with none (no look in the audit needs it), the eye-clear spot that overreaches
   least.

The **eye box** is the bible's: the eye's largest box (`rig.info.eye`: the eye in any state and a 1 px ring), placed
in face space as the rig places it (`eyeBox`). A plan takes about 45 ms on average (at most about 160) and is cached
...
| Check | What fails |
|---|---|
| `npm run palette`, KEEPERS | any of the 144 keeper gates (3) |
| `view=careaudit` (smoke) | every care act on all 28 looks, 112 runs (Bea feeds, Tomas grooms, Pip pets, Iris tucks in): a keeper at work covering the dragon's eye box by one pixel, a stroking hand more than 2.5 px off its mark, an act that never ends |
| `view=yardaudit` (smoke) | the same for every act the yard plays in 9000 frames |
...
At the last run: all 112 care runs and the yard's 25 acts eye-clear, the worst hand miss 2.0 px (the yard's 1.6).
...
- **The tiptoe and the cheer covered the keeper's own face** (the near shoulder sits forward, so a raised near arm
  crosses the face; both arms out in front read as a sleepwalker): a finger to the lips, and a fist pumped in front.
- **Walking**: a keeper whose x arrived before its depth drifted back and forth; one walking from the kitchen to feed
  a baby walked over it. Depth now arrives with the walk and a keeper goes round behind.
