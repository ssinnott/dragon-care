...
| scared | the surprised eye (babies too) | front end up 2 px | jaw closed; a 1 px tremble every 4 f. With shriekscale's fans at −1. |
...
- **Neutral area stays ≤ 40 %.** The neutral slots are small: spike's bone quills, rock's cream belly, water's pearl horn and spots, shriekscale's belly and ribs. No body colour is neutral (rock is the lowest, at S 0.42).
...
- **Effect ownership.**
  ...
  - `event: 'shriek'` flinches are capped at 2 neighbours. The lonely call (`event: 'call'`) makes neighbours look, not flinch (3.7). Both events are raised by shriekscale's anims; nothing consumes them yet (the game's pet loop will).
...
- **The habitat check** (5.1 #13) carries the risky looks and holds two overlaps in place: adult lightning, the tallest cue, standing just behind an adult shriekscale (spire behind fan), and an adult water with its baby in front of it (parent over baby, one element); with a young rock, a young fire and spike's adult and baby roaming. Both overlaps separate by outline and colour at game scale, idle and in the mix.
...
- **Habitat floor constraint** (gate i).
  ...
  - That forces every belly to sit ≤ 0.49 or ≥ 0.89 in luminance: fire, spike, lightning and water have mid-value bellies, rock and shriekscale near-white ones. The first draft's pale bellies sat 2 to 21 % from this floor on five of the six.
...
(b) SCALE COLOURS ACROSS ELEMENTS  (B1 hue: both S>=0.30 and >=40deg | B2 value: >=25% | B3 chroma: dS>=0.30 and >=12%)
  ...
  ok   rock/shriekscale       #d4b67c #5a2f6e  lum  89%  hue 119deg  dS 0.16  B1+B2
 ...
 scale S/V: fire 0.86/0.94  spike 0.64/0.51  rock 0.42/0.83  lightning 0.78/0.80  water 0.84/0.81  shriekscale 0.57/0.43
...
(f) COLOUR-BLIND SAFETY  (simulated deuteranopia / protanopia, Vienot 1999: 15 scale pairs by RULE_B, blush/scale by the ladder)
 deutan
  ...
  ok   rock/shriekscale       #bfbf7b #3f3f6d  lum  89%  hue 180deg  S 0.36/0.42  B1+B2
 ...
 protan
  ...
  ok   rock/shriekscale       #baba7c #36366e  lum  90%  hue 180deg  S 0.33/0.51  B1+B2
...
### Element pass
Each element was finished by its own artist in its own worktree (branch `elem/<id>`), critiqued, and fixed there, touching only its `elements/<id>.ts`: fire `e5252ed`, spike `7cda58c`, rock `a342021`, lightning `6390a80`, water `e8e69e8`, shriekscale `e833246` (its review-fix pass finished after a restart). The six files were integrated as they are; what each element now draws is in section 3 and 4.3, from the artists' notes. The artists' requests for SHARED changes were then applied or turned down, and the lead art director's own findings on the renders fixed, each looked at on game-scale renders before and after (lineup idle / walk / sleep, the silhouette sheet with its ÷ 3 blocks, the habitat and the habitat mix, every element's stage sheet, and the strips, zooms and casts named below); `tsc`, the palette check (277 / 277) and the smoke run (37 views) pass, and the floor audit, now over the element anims too (249 runs a seed), finds nothing sinking on seeds 1, 2, 3 and 5.

**Shared requests.**

| # | Request | From | Verdict | Why, in one line |
|---|---|---|---|---|
| 1 | The looping pet and beg clocks run backwards | fire, rock, water, shriekscale | Applied | `cue: (t) => t` in both (4.1): bake folded the [L, L] key onto frame 0 of a wrapping loop. Checked on the baked frames (cue = frame time, all four); fire's and shriekscale's own re-keying is removed, spike's growl sag now lands on the growl. |
| 2 | Shared act ids: `bath`, `upset`, `call` | fire, rock, shriekscale | Applied | Three asked for id 11: `ACT.bath` 11, `ACT.upset` 12, `ACT.call` 13, the local constants replaced; `bath` and `call` are one-shots, all three in the floor audit and in `npm run shots`. Bath, upset and call strips re-rendered unchanged. |
...
| 13 | `neckSac`: re-centre it or remove it | shriekscale | Applied: removed | Nothing uses it: shriekscale draws its own sac 0.6 up the neck, behind the head, which shows; a hook centred under the jaw never could (1.2). |
| 14 | `sleep.frontTuck` in tuning | shriekscale | Applied | Where the forepaws rest asleep; shriekscale sets 10 and drops its sleep and wake overrides. The sleep and adult wake strips are pixel-identical; the young wake's third frame reaches a little less far forward. |
| 15 | The "z" overlaps the baby shriekscale's ear | shriekscale | Applied | Folded into (b) below. |
| 16 | `facing=-1` for zoom and strip | rock | Applied | And `bond`, `charge`, `pupil` in the gallery query. |
| 17 | `upset` / `call` / `bath` in the floor audit | rock, shriekscale, fire | Applied | With #2. |
...
| 19 | The game consumes `event: 'shriek'` and `'call'` | shriekscale | Not now | No game loop exists; the events are raised and documented (3.7, 5.4). |
...
- Shriekscale: at rest its ribs read as diverging strokes rather than fingers; the baby's rib looks a little like a gloss spot at mood 0; rib length shifts by one end stamp with the idle breath; its hand-mapped fan ribs would not follow a looked-back head (no shriekscale anim looks back).
...
| S9 | The habitat check misses the risky looks and overlaps | Applied | Adult lightning just behind an adult shriekscale and an adult water over its baby, both fixed; young rock in (5.4). |
...
- Shriekscale: at rest its adult ribs read as diverging strokes rather than fingers (A10).
