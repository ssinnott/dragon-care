...
- **6 elements:** fire, spike, rock, lightning, water, slinkwing.
...
**Status.** This is the design, and the rig now draws it: the quadruped rig, the shared features and all six elements, each finished by its own element artist, reviewed and integrated (the **Element pass** in the Review log), live in `src/art/dragon/` (file list in 1.1). Built: the core animation set of 4.2 per stage (idle with its variants, walk, happy, eat, sleep and wake, breath, pet, beg), authored as tracks from the 4.1 stage timing rules and tuned per element by the generic knobs of 4.3; every element's signature breath, baby fizzle, idle fidget (section 3) and 4.3 column (walk, happy flourish, hungry tell, sleep pose); the element anims fire's `bath`, rock's `upset` tuck and slinkwing's lonely `call`; rock's roll-over happy and the E8 fan flop; a turn in place (a paper turn: 4.1) and a head that looks back over its shoulder (a head pitched past vertical: 4.1). **Not built yet:** the anims grow-up, hopGlide, fly and rock's boulder hop, play, refuse and zap, and the other elements' baths; water floating belly-up (the habitat has no water) and spike's shed quills (a collectible). The game's side of the care loop does not exist yet: nothing consumes `event: 'shriek'` or `'call'`, the pebble pile of rock's hungry tell is drawn by rock rather than placed as a habitat prop, and `bond`, `charge` and `wary` (DrawDragonOpts) are fed only by the gallery (its `bond` / `charge` query and the habitat's wary latch). Each element file's header lists what it draws. The palette is code too: `src/art/dragon/palettes.ts` holds the values, and `tools/palette-check.ts` validates them (report in section 5.8). `npm run shots` renders the standard sheets of 5.5 and `npm run smoke` checks every gallery view, the floor audit and the leg-root audit included. Every hex below comes from that module and passed that check. The bible has been through one design review round (a readability critique and a care-game critique), four review rounds of the built rig, the element pass and a first cast review (the whole cast judged together); what changed, and what was turned down, is in the **Review log** at the end, with what building the core animations found.
...
|---|---|---|---|
| D1 | **One element per silhouette zone:** fire above the tail tip, spike the back line, rock the body mass, lightning the space above the back (behind the head), water the tail end, slinkwing the head. Adopted with C's **quiet-zone rule** and G's **zone budgets** (3.0). | R + C + G | C paired elements in a zone, sharp against soft; at 24 px, "sharp vs soft" in the same place collapses into the same blob. G put three elements on the head (bolt horn, ears, cheek fins); an adult head is about 6 px long at 24 px and holds one feature, not three. |
...
| | jaw maximum opening | 40° | 34° | 30° (slinkwing 40°) |
...
| | **finger spars, leading → trailing** (radius) | none | **14 / 11** (r 1); 3-spar wings (water, slinkwing) **14 / 11 / 9** | **20 / 17 / 13** (r 1.5 → 1); 4-spar wings (water, slinkwing) **20 / 17 / 14 / 11** |
| | folded silhouette / spread tip height above the root | 7 × 5, cannot fold flat: rests at 170° with a ≈ 4 × 3 knuckle showing behind the head, 140° in flutter and happy / flutters ±35° | ≈ 15 × 8 (the knuckle, the leading edge and the panel under it: 1.3) / ≈ 27 | ≈ 21 × 11 / ≈ 40 |
...
| | Fire | Spike | Rock | Lightning | Water | Slinkwing |
|---|---|---|---|---|---|---|
| **Adult** | 107 × 55 | 100 × 51 | 93 × 48 | 105 × 64 | 123 × 55 | 110 × 70 |
| **Young** | 80 × 45 | 77 × 41 | 72 × 38 | 80 × 47 | 92 × 45 | 84 × 55 |
| **Baby** | 41 × 36 | 43 × 33 | 38 × 30 | 38 × 36 | 50 × 33 | 45 × 41 |
...
- **Stage steps.** Within an element, every stage step is at least 1.29× in length (rock, young → adult) and 1.22× in height (fire and water, young → adult; baby → young at least 1.24×, spike), so a stage is never mistaken for its neighbour. These are the floors 5.1 #3 checks against. The head ratio, eye and neck carry the rest.
...
| Slot | Fire | Spike | Rock | Lightning | Water | Slinkwing |
|---|---|---|---|---|---|---|
| **scale** | `#f04422` | `#2f8232` | `#d4b67c` | `#2d58cc` | `#20a3ce` | `#5a2f6e` |
| **belly** | `#e08a2c` | `#a3ad55` | `#fff5cc` | `#9fb4f2` | `#85c6ae` | `#fff5f8` |
...
- **membrane:** the wing membrane, plus the element's skin features (water's fluke, fins and fin-ears; slinkwing's fans and throat sac), slinkwing's sound arcs, and the 1 px ring on water's bubbles and drips.
- **horn:** horns, claws (un-inked), quills, fan ribs and fin rays.
- **marking:** element markings, rock's dome and slinkwing's eye mask.
...
| far leg scale (0.55 / 0.30) | `#702d26` | `#224229` | `#706554` | `#203063` | `#22546b` | `#2e1d3b` |
...
**Colour rules:**
- **The value ladder is the same in every palette:** belly lighter than scale, membrane darker than scale (and darker than the scale's shadow tone, except spike's: D11), except where the membrane *is* the signal colour (lightning, slinkwing).
...
- **One saturated signal family per dragon.** Glow is the signal. Lightning uses its yellow for membrane, stripes and eye as well, one hue family. Slinkwing's pink is membrane and glow. Glow is drawn flat and never banded.
...
- **Neutral area stays ≤ 40 %.** The neutral slots are small: spike's bone quills, rock's cream belly, water's pearl horn and spots, slinkwing's belly and ribs. No body colour is neutral (rock is the lowest, at S 0.42).
...
| Markings: flame-lick chevrons in `marking`, pointing up: chunky filled carets (5.2), SOLID (no notched foot: notched, the adult's three read as the letters "A A A") | 1 on the **tail base**, 4 × 4 (the baby's shoulder is under its head) | 5 × 5: tail base + haunch | 6 × 6: tail base + haunch + shoulder |
...
|---|---|---|---|
| Quills (count × height, base) | 3 soft nubs, 6 tall on a base of 5 (the tail's 5 tall), their points rounded 1 px, on the **loin, rump and tail**: the loin and rump nubs 6 px apart centre to centre (body x −2.1 and −8.1 on the baby's hip), the tail nub 60 % along the short tail, clear of the rump one. The 4 × 4 blunt nubs never covered half of a 3 × 3 cell and were gone at /3 in every block (the baby a blob with a tail stub beside baby rock and baby water); the baby's head covers the front 60 % of its back, so nubs at the neck base and shoulders would be hidden. Lean at half the adult range (25° → 14° → 7°) | 3 on the back [8, 10, 7], base 6, + 1 on the tail (5). Four teeth over the young's short back sat 4.5 px apart, 1.5 px at /3, and merged into a flat edge (5.1 #1); three at about 7 px keep a saw at /3 even at `mood` −1 (at [7, 9, 6] the ÷ 3 saw was 1 px nicks at rest too). **Asleep** [6, 7, 8] + 6: graded against the back line, so the tips run level | 5 on the back [7, 11, 12, 10, 7], base 8 (neck base, shoulders, mid-back, loin, rump), + 3 on the tail [6, 5, 4], bases 6 / 5 / 4. **Asleep** tall and short by turns, [10, 5, 11, 5, 12] + [7, 6, 5] on the tail: the tall three stand 11 px apart with their tips level (the back line rises toward the shoulders, so the rump's is tallest), a crown of 3 teeth at ÷ 3 at every phase; 5.5 px apart on overlapping 8 px bases, the awake heights peaking mid-back made a crest of 1 px nicks on a mound, the closest pair on the sheet to rock's dome. Wide bases: at 5 to 6 px a leaning quill left 1 to 2 px of bone between two ink edges and read as a grey scribble. All in plain `horn`: no dark tips (a 3 px tip on a tapering quill is under 2 px wide and merges with the ink) |
| Brow thorns: 2, pointing back, lying back along the neck line, rooted on the cranium's **back** contour (145°) as fire's horns are (rooted on its top, at 105°, they stood up like ears; then, lying back, they lay along the skull's own top and read as a pale strap or goggles), tapering to a point (r → 0.5: with a round 2 px end the thorn read at game scale as a pale capsule, a hair clip or a bandage), and never more than 20° above or 40° below straight back in the world (3.0) | none | 3 px, r 1.5 → 0.5 (the far one culled: one thorn) | 6 px, r 1.5 → 0.5; the far thorn forks at 8° (`farTilt`), so the overlap cull removes it and one thorn shows, as on the young (at the adult default 28°, and still at 16–20°, it lay over the near one as a flat grey slab, a hair clip) |
...
| Tail | thin, straight and stiff, 0.65 as long, lifted a perky 12°; a plain taper to a true point (tip r 0.5); motion is stepped (4 f holds) | the same, full length, lifted 5° (level it was a ruler-straight rod, and its round-capped tip read as a spear's end) | same |
...
| Feature | Baby | Young | Adult |
|---|---|---|---|
| Ear-fans, h × w, ribs, edge | 10 × 8 fennec leaf, 1 rib, on top of the cranium | 13 px tall, 2 ribs as 1 pleat, 1 scallop 3 px deep on the near fan | 17 px tall, 3 ribs as 2 pleats, 2 scallops 3 px deep on the near fan |
...
| Throat sac: a `membrane` bulge 0.6 of the way up the neck from its root, along the throat's normal, drawn by the element at its breath anchor and clipped behind the head; inked only outside the neck's contour (over the neck's skin it is a colour change); shown while shrieking and through the lonely call | swells 3 px | 5 px | 7 px |
| Legs | thin, bat-like (leg r × 0.75): flat at every stage, so the far leg never has to clear a shadow band (D8) | same | same |
| Jaw | depth 1.3, opens to 40° (the element's jawMax 40 replaces the young's 34) | same | same |
| Tail | a whip (r × 0.7), plain taper, no barb | same | same |
| Wings | nub | bat, 3 spars, 3 px scallops | bat, 4 spars, 3 px scallops (at 5 px its four dark spars stood well past the membrane, and the spread wing read as a raised hand, a rake), 3 px thumb claw |
| Markings | eye mask | eye mask + one 5 × 4 "volume-bar" chevron on the tail at t 0.4 | eye mask + a 6 × 5 chevron at t 0.3, then a 5 × 4 at t 0.5, stepping down along the whip ("3 px" is the chevron's band thickness, 5.2) |
